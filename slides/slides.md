...
| 手段 | 宛先 | 用途 | 状態 |
|---|---|---|---|
| **Cloudflare Email Service** | 任意ユーザー OK | パスワードリセット・登録確認・マーケ | **Public Beta**（2025/10〜）|
| Email Routing `send_email` | 検証済み宛先のみ | 管理者通知・内部アラート | GA（無料） |
| 外部 API (Resend/SES 等) | 任意ユーザー OK | 既存資産を活かすケース | GA |

### Cloudflare Email Service の強み
- **API キー管理なし**（Workers バインディング直挿し）
- **DNS (SPF / DKIM / DMARC) 自動設定**
- Resend / SES 相当の DX、Cloudflare 内で完結
