...
### 2026年、Cloudflare 内で完結できるようになった 🎉
...
### 従来の SMTP / 外部 API と比べて何が嬉しい？

- **API キー管理なし**
  - Workers バインディング `env.SEND_EMAIL.send()` で直接送信
  - Secrets Manager / Vault 連携が不要

- **DNS (SPF / DKIM / DMARC) 自動設定**
  - ドメイン verify の一手間だけで deliverability が担保される

- **Cloudflare 内で完結**
  - 外部 API の latency / 障害点がひとつ減る
  - 課金もまとめて Workers プランに乗る

> DX は Resend / SES 相当、運用は Workers のまま

<!--
Workers Paid プラン限定という制約はあるが、運用上のメリットが大きい。
特に API キー管理不要は、セキュリティ担当者への説明コストを下げられる。
DNS 自動設定は SES で手作業だった SPF/DKIM/DMARC 設定の面倒さを知ってる人には刺さる。
-->
