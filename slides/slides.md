...
### 本日のサンプル
- GitHub: `https://github.com/SuguruOoki/cloudflare-php-demo`
- Live: `https://cloudflare-php-container-demo.suguru-ohki.workers.dev`
...
> 迷ったら B。実プロダクト投入は B なら活用できる資産も多い
...
```php
<?php
// public/index.php  ← これを書くだけで Worker 上で動く
...
### デプロイも1コマンド
```bash
wrangler deploy   # ← 世界330都市に即デプロイ
```

php-wasm が Worker 内でこの `index.php` を実行する（ランタイムは自動で bundle）

<!--
TS shim は bundler が自動生成するので PHPer は書かない。
PHP ファイルを書いて wrangler deploy、だけで世界330都市に配布。
"330都市" は聴衆が「おっ」となるキーワード。
-->
...
- User A の応答が **User B** にそのまま返る → **個人情報漏洩 / 認証バイパス**
...
| **認証 GET**（自分のデータ） | `Authorization` / セッションあり → **bypass** |
| **認証 GET を効かせたい稀ケース** | **cache key にユーザーID**（`cf.cacheKey`） |

<!--
この話を抜かすと「Cloudflare 使ったら情報漏洩した」系の事故につながる。
次ページで二重防御の実装（Laravel Cache-Control + Worker ガード）を見せる。
-->
...
### Laravel 側: Cache-Control を明示
```php
return response()->json($data)
    ->header('Cache-Control', 'private, no-store');
```

### Worker 側: shouldBypassCache() で迷ったら bypass
```ts
export function shouldBypassCache(req: Request): boolean {
  if (req.method !== 'GET' && req.method !== 'HEAD') return true;
  if (req.headers.has('Authorization')) return true;       // 認証あり
  const cookie = req.headers.get('Cookie') ?? '';
  if (/laravel_session|auth/i.test(cookie)) return true;   // セッションあり
  return false;
}
```
...
の二重防御が基本。
実装は apps/frankenphp-container/src/handlers/cache.ts に withEdgeCache() がある。
-->
...
## Email Service ②: Laravel から送る

```php
// 既存の Laravel コードはそのまま
...
### 裏側: Mail カスタム Transport（AppServiceProvider::boot）
```php
...
```
Transport 内部で **Worker の `/api/send-mail` に HTTP POST** → Worker 側で `env.SEND_EMAIL.send()` が走る

<!--
Laravel からは "いつもの Mail::to()->send() がそのまま動く" が肝。
Worker 側の TS は apps/frankenphp-container/src/handlers/email.ts 参照。
Transport パターンは Symfony Mailer (Laravel 内部実装) のやり方に合わせる。
-->
...
## Queue 実装 ②a: Laravel から投入する

```php
// いつもの Laravel Queue — ドライバだけ CF 向けに切替
...
class GenerateMonthlyReport implements ShouldQueue
{
    ...
    public function handle(): void { /* PDF 生成など重処理 */ }
}
```

→ `dispatch()` の瞬間に **Worker の `/api/enqueue` に HTTP POST** され、即リターン

<!--
「既存の Job クラスがそのまま動く」が最大の訴求ポイント。
Laravel Queue カスタムドライバ (cloudflare) 内部で Http::post するだけ。
-->

---

## Queue 実装 ②b: Laravel で処理する

```php
// routes/web.php  (Consumer Worker から HTTP で呼ばれる)
Route::post('/process', function (Request $request) {
    $payload = $request->json()->all();

    // いつもの Laravel Job ハンドリング
    match ($payload['kind']) {
        'monthly-report' => GenerateMonthlyReport::dispatchSync(
            $payload['args']['user_id']
        ),
        'welcome-mail'   => Mail::to($payload['args']['email'])
...
→ **重い処理は Laravel 内で完結**（CPU 5分 / wall 15分の実行枠）、失敗時は Worker が自動リトライ → DLQ

<!--
Cloudflare Queue Consumer は Worker 側 (TS) だが、本体処理は Laravel に HTTP POST で委譲。
PHPer から見れば "いつもの Job::handle() がそのまま動く" 構造。
Worker の TS は apps/frankenphp-container/src/handlers/queue.ts 参照。
-->
