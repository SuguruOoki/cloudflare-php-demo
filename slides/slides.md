...
## batch_size / batch_timeout: 配送の粒度

### 2つのダイヤル
- **`max_batch_size`**: Consumer が 1 回で受け取る最大件数（既定10 / 最大100）
- **`max_batch_timeout`**: バッチが埋まらなくても配送される待ち秒数（既定5 / 最大30）

### 具体例（size=10 / timeout=30）
| シナリオ | 結果 |
|---|---|
| 10件が2秒で溜まる | **即配送**（timeout を待たず満杯で送る） |
| 3件のまま30秒経過 | 30秒後に **3件だけで配送** |
| 0件のまま | Consumer は呼ばれない（ゼロコスト） |

### 30秒 CPU 上限との相性
`batch_size × 1件処理時間 < 25秒` に収める。Container 側に委譲するなら batch は大きめでも OK。

<!--
よく質問が出るポイント。batch_size と timeout はどちらかだけで決まらず「両方で配送条件」になるのが肝。
10件揃えばtimeout無視で即配送、という点を強調すると理解が進む。
-->
...
### DLQ 無しだと
- 失敗メッセージが**本線キューを詰まらせ**、正常ジョブの処理がブロック
- 何が失敗したか後から追えない
...
### Laravel ユーザー向け一言
**Laravel の `failed_jobs` テーブルとほぼ同じ役割**。違いは DLQ 自体もキューなので、「失敗ジョブ再実行」を Consumer 追加だけで実装できる。

<!--
比較表的な見せ方で「DLQ の運用価値」を訴求。
failed_jobs 類比は Laravel ユーザーに即伝わる。
事前に wrangler queues create xxx-dlq で本線と DLQ の2本を作る運用 tips も口頭で添える。
-->
