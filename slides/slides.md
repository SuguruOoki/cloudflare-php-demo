...
### DLQ 無しだと起きること
- 失敗メッセージが本線キューを詰まらせ、正常ジョブの処理がブロック
- 何が失敗したか後から追えない

### DLQ があると
- 失敗を別キューに隔離、本線を守る
- 後から DLQ を覗いて原因分析
- 修正後、DLQ → 本線に**再投入**も可能（DLQ 自体も Consumer を紐づけられる）

### Laravel ユーザー向け一言
**Laravel の `failed_jobs` テーブルとほぼ同じ役割**。違いは DLQ 自体もキューなので、「失敗ジョブ再実行」機能を Consumer を追加するだけで実装できる。

<!--
DLQ は登壇聴衆から質問が出やすいキーワード。1ページ割いて説明する価値あり。
「failed_jobs と同じ」の1行で Laravel ユーザーには即伝わる。
事前に wrangler queues create xxx-dlq で本線と DLQ の2本を作っておく必要がある、
という運用 tips も口頭で触れる。
-->

---

## Queue 実装 ②: Worker コード (Producer + Consumer 同居)
...
**Laravel**: `QUEUE_CONNECTION=cloudflare` カスタムドライバで `Queue::push(new Job)` がそのまま動く

<!--
1つの worker.ts に Producer/Consumer を同居できるのが CF Queues の美味しい所。
Laravel ユーザーには「いつもの Queue::push がエッジで動く」で十分通じる。
時間が押したらこのスライドは 30秒で流して次へ、質問が出たら深掘りで補足。
-->
