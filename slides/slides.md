...
### Cloudflare Email Routing で**部分的に**送信できる
Worker の `send_email` binding 経由で送れる。ただし以下の制約:

| できる | できない |
|---|---|
| ✅ **検証済み宛先**に送信（自分・運用チーム） | ❌ 任意のユーザー宛 |
| ✅ Email Routing 設定ドメインから送信 | ❌ 汎用 SMTP 代替 |

### 用途の切り分け

- **管理者通知 / エラー通報 / 内部アラート** → **Email Routing でOK**（無料、CF内で完結）
- **パスワードリセット / 会員登録確認 / マーケ** → **外部 API が必須**

<!--
Email Routing は 2024 以降で Email Workers + send_email binding により
"検証済み宛先に送信" が可能になった。完全な受信専用ではない点に注意。
ただし "事前に verify した宛先にしか送れない" ので、任意ユーザー宛メールには使えない。
この切り分けが PHPer への正確な説明になる。
-->

---

## ユーザー向けメール: 外部 API を使う
...
### 結論: 送信はハイブリッド運用
- **内部宛て** → Cloudflare `send_email` binding（無料 / ゼロ設定）
- **ユーザー宛て** → 外部 API（Resend 等、Laravel Mail そのまま）

<!--
Resend は 2024-2025 に Laravel 公式ドライバが入り DX 最良。会場で一言推しても良い。
「送信手段はハイブリッドに分ける」が現実解。
内部通知まで外部APIに載せるのはコスト面でも無駄なので、2系統使い分けるのが賢い。
-->
