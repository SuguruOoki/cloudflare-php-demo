...
| 制限 | Workers HTTP (fetch) | Queue Consumer | Containers |
|---|---|---|---|
| **CPU time** | **30秒** | **既定30秒 / 設定で最大5分** | 実質無制限（常駐） |
| **Wall clock** | 〜15秒でクライアント切断 | **最大15分** | 無制限 |
| **クライアント応答** | 処理完了まで待つ | **即 202 応答**（非同期） | 処理完了まで待つ |
| **失敗時** | 呼び出し元で対処 | **自動リトライ + DLQ** | 5xx を返す |
| **バッチ処理** | 1件ずつ | **最大100件まとめて** | 1件ずつ |
...
| キー | 意味 | 推奨値 | 上限 |
|---|---|---|---|
| `max_batch_size` | 1 consumer 呼び出しで受け取る最大メッセージ数 | 10〜100 | 100 |
| `max_batch_timeout` | バッチが埋まらなくても何秒で配信するか | 30 | 60 秒 |
| `max_concurrency` | 同時並行 consumer 数 | 2〜10（下流負荷に合わせる） | 250 |
| `max_retries` | 失敗時の最大リトライ回数 | 3〜5 | 100 |
| `retry_delay` | リトライまでの秒数（`msg.retry({delaySeconds})` でも指定可） | 60 | 24 時間 |
| `dead_letter_queue` | N回失敗後の退避先 | 必ず設定する | — |

### Consumer ハンドラの実行時間枠（重要）

| 枠 | 値 |
|---|---|
| **CPU time / 1 invocation** | 既定 **30秒** / 設定で最大 **5分** |
| **Wall clock / 1 invocation** | 最大 **15分** |

HTTP fetch ハンドラの 30秒制限と違い、Queue Consumer は**重い処理を Worker 単体で完結**できる余裕がある。Container に委譲せず Worker 内で PDF 生成・一括メール等もここで走らせられる。
...
Queue Consumer は CPU 最大 5 分 / wall 最大 15 分 使えるので:

- **Worker 内で PHP 処理を完結**: `batch_size × 1件の処理時間 < 5分` に収める（CPU枠）。I/O 待ちが多いなら wall 15 分まで伸ばせる
- **Container にフォワード**する場合: Worker 側の CPU 上限はほぼ消費しないので batch は大きめで OK
